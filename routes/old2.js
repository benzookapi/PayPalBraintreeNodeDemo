...
router.get('/', function(req, res, next) {
  ...
  gateway.clientToken.generate({}, function (err, response) {
    res.render('old2', { clientToken: response.clientToken });
  });
});
  ...
  gateway.transaction.sale(saleRequest, function (err, result) {
    if (err) {
      ...
      res.send("<h1>Success! Transaction ID: " + result.transaction.id + "</h1><br/>" +
        ...
        "<br/><br/><a href=\"/old2\">Try again</a>" +
        "<br/><br/><a href=\"/old2/cap_trans?amount=" + req.body.amount + "&transactionId=" + result.transaction.id + "\">Try Capture</a>");
    } else {
      res.send("<h1>Error:  " + result.message + "</h1><br/>" + JSON.stringify(result, null, 4).replace(/\n/g, "\n<br/>").replace(/ /g, " &nbsp;") + "<br/><br/><a href=\"/old2\">Try again</a>");
    }
  });
...
　});
  ...
  gateway.transaction.sale(saleRequest, function (err, result) {
    if (err) {
      res.send("<h1>Error:  " + err + "</h1><br/>" + JSON.stringify(result, null, 4).replace(/\n/g, "\n<br/>").replace(/ /g, " &nbsp;") + "<br/><br/><a href=\"/old2/vault\">Try again</a>");
    } else if (result.success) {
      res.send("<h1>Success! Transaction ID: " + result.transaction.id + " Customer ID: " + result.transaction.customer.id + "</h1><br/>" +
        JSON.stringify(result, null, 4).replace(/\n/g, "\n<br/>").replace(/ /g, " &nbsp;") +
        "<br/><br/><a href=\"/old2/vault\">Try again</a>" +
        "<br/><br/><a href=\"/old2/vault_sale?customerId=" + result.transaction.customer.id + "\">Try Vault Sale</a>");
    } else {
      res.send("<h1>Error:  " + result.message + "</h1><br/>" + JSON.stringify(result, null, 4).replace(/\n/g, "\n<br/>").replace(/ /g, " &nbsp;") + "<br/><br/><a href=\"/old2/vault\">Try again</a>");
    }
  });
    ...
    gateway.customer.create(csRequest, function (err, result) {
      if (err) {
        ...
        res.send("<h1>Success! Customer ID: " + result.customer.id + "</h1><br/>" +
          JSON.stringify(result, null, 4).replace(/\n/g, "\n<br/>").replace(/ /g, " &nbsp;") +
          "<br/><br/><a href=\"/old2/vault\">Try again</a>" +
          "<br/><br/><a href=\"/old2/vault_sale?customerId=" + result.customer.id + "\">Try Vault Sale</a>" +
          "<br/><br/><a href=\"/old2/vault?customerId=" + result.customer.id + "\">Update Customer</a>");
      } else {
        res.send("<h1>Error:  " + result.message + "</h1><br/>" + JSON.stringify(result, null, 4).replace(/\n/g, "\n<br/>").replace(/ /g, " &nbsp;") + "<br/><br/><a href=\"/old2/vault\">Try again</a>");
      }
    });
...
　});
  ...
  gateway.transaction.sale(saleRequest, function (err, result) {
    if (err) {
      ...
      res.send("<h1>Success! Transaction ID: " + result.transaction.id + "</h1><br/>" +
        JSON.stringify(result, null, 4).replace(/\n/g, "\n<br/>").replace(/ /g, " &nbsp;") +
        "<br/><br/><a href=\"/old2/vault_sale?customerId=" + req.body.customerId + "\">Try Vault Sale</a>" +
        "<br/><br/><a href=\"/old2/vault\">Try again</a>");
    } else {
      res.send("<h1>Error:  " + result.message + "</h1><br/>" + JSON.stringify(result, null, 4).replace(/\n/g, "\n<br/>").replace(/ /g, " &nbsp;") + "<br/><br/><a href=\"/old2/vault\">Try again</a>");
    }
